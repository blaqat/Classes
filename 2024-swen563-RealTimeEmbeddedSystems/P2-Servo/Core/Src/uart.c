...
void USART2_Init(int baudrate) {
  ...
  tmpreg =
      RCC->APB1ENR1 &
      RCC_APB1ENR1_USART2EN; // Delay after an RCC peripheral clock enabling

  // enable GPIOA CLK
  RCC->AHB2ENR |= RCC_AHB2ENR_GPIOAEN;
  tmpreg = RCC->AHB2ENR &
           RCC_AHB2ENR_GPIOAEN; // Delay after an RCC peripheral clock enabling
  ...
  USART2->CR1 &= ~(USART_CR1_M1_Pos | USART_CR1_PCE_Msk | USART_CR1_OVER8_Msk |
                   USART_CR1_TE_Msk | USART_CR1_RE_Msk);
  USART2->CR1 |= UART_WORDLENGTH_8B | UART_PARITY_NONE | UART_OVERSAMPLING_16 |
                 USART_CR1_TE | USART_CR1_RE;

  /*-------------------------- USART CR2 Configuration -----------------------*/
  /* Configure the UART Stop Bits: Set STOP[13:12] bits according to
   * huart->Init.StopBits value */
  USART2->CR2 &= ~USART_CR2_STOP;
  USART2->CR2 |= UART_STOPBITS_1;

  /*-------------------------- USART CR3 Configuration -----------------------*/
  /* Configure
   * - UART HardWare Flow Control: set CTSE and RTSE bits according to HwFlowCtl
   * value
   * - One-bit sampling method versus three samples' majority rule according to
   * huart->Init.OneBitSampling */
