...
int test_student()
{
	...
	assert(result == 1 && list1.p_current->one_word.word_count == 3,
		"Expected count of the word 'day' 3 times but got &d.", result==-1?0:result) ? passcount++ : failcount++;
	...
	assert(strcmp( list1.p_current->p_next->one_word.unique_word, "heat" ) == 0,
		"Expected the word 'have' after 'heat' got %s", &list1.p_current->p_next->one_word.unique_word) ? passcount++ : failcount++;
	
	// Add a test to verify that the last word in the list is "with"
	struct word_entry re = get_last_word(&list1);
	assert(strcmp(re.unique_word, "with") == 0, "Expected the last word to be 'with' got %s", re.unique_word) ? passcount++ : failcount++;
	
	// Add a test to verify that the next to the last word is "will"
	re = get_prev_word(&list1);
	assert(strcmp(re.unique_word, "will") == 0, "Expected the last word to be 'will' got %s", re.unique_word) ? passcount++ : failcount++;
