...
void clock_init(void) {
  // Enable HSI16 which is the default clock after reset
  ...
  RCC->CFGR |=
      (uint32_t)RCC_CFGR_SW_HSI; // 01: HSI16 oscillator used as system clock
...
void SysClock_Init(void) {
  ...
  // RCC_ICSCR
  HSITrim = 16; // user-programmable trimming value that is added to HSICAL[7:0]
                // in ICSCR.
  ...
  RCC->PLLCFGR |=
      RCC_PLLCFGR_PLLSRC_HSI; // 00 = No clock, 01 = MSI, 10 = HSI, 11 = HSE
  ...
  RCC->PLLCFGR =
      ...
      1U << 4; // 000: PLLM = 1, 001: PLLM = 2, 010: PLLM = 3, 011: PLLM = 4,
               // 100: PLLM = 5, 101: PLLM = 6, 110: PLLM = 7, 111: PLLM = 8

  RCC->PLLCFGR &= ~RCC_PLLCFGR_PLLR;  // 00: PLLR = 2, 01: PLLR = 4, 10: PLLR =
                                      // 6, 11: PLLR = 8
  ...
  RCC->CFGR &=
      ~RCC_CFGR_PPRE1; // APB high-speed prescaler (APB1) = 1, HCLK not divided
  RCC->CFGR &=
      ~RCC_CFGR_PPRE2; // APB high-speed prescaler (APB2) = 1, HCLK not divided
  ...
  // enable RCC->PLLCFGR |= RCC_PLLCFGR_PLLQEN; // Enable Main PLL PLL48M1CLK
  // output enable
